...
public class StaffDAO extends DBContext{
    ...
    public ArrayList<Staff> getStaff() {
        ...
        try {
            String strSQL = "select * from Users s join Roles r on s.id_role = r.id";
            ...
            while (rs.next()) {
                ...
                Staff p = new Staff(id, name, account, password, email, sdt, address, role);
                data.add(p);
            }
        } catch (Exception e) {
    ...
    public void update(Staff s) {
        try {
            String strSQL = "update Users set username=?,account=?,password=?,email=?,sdt=?,address=?,id_role=? where id=?";
            stm = connection.prepareStatement(strSQL);
            ...
            stm.setInt(7, Integer.parseInt(s.role));
            stm.setInt(8, Integer.parseInt(s.id));
    ...
    try {
        ...
        String strSQL = "INSERT INTO Users(username, account, password, email, sdt, address, id_role) VALUES (?, ?, ?, ?, ?, ?, ?)";
        stm = connection.prepareStatement(strSQL);
        stm.setString(1, s.name);
        stm.setString(2, s.account);
        stm.setString(3, s.password);
    ...
    public boolean isAccountExist(String account) {
        ...
        try {
            String strSQL = "SELECT COUNT(*) FROM Users WHERE account = ?";
            stm = connection.prepareStatement(strSQL);
