                        ...
                        <div class="collapse navbar-collapse" id="navbarCollapse">
                            <div class="d-flex justify-content-between align-items-center w-100">
                                <div class="navbar-nav mx-auto">
                                    ...
                                    <a href="feedback.jsp" class="nav-item nav-link active">Phản Hồi</a>
                                </div>
                            </div>
                        </div>
                    </nav>
        ...
        <div class="container-fluid page-header" style="margin-bottom: 90px;">
            <div class="container">
                <div class="d-flex flex-column justify-content-center" style="min-height: 300px">
                    <h3 class="display-4 text-white text-uppercase">Phản Hồi</h3>
                    <div class="d-inline-flex text-white">
                        <p class="m-0 text-uppercase"><a class="text-white" href="HomePage">Trang Chủ</a></p>
                        <i class="fa fa-angle-double-right pt-1 px-3"></i>
                        <p class="m-0 text-uppercase">Phản Hồi</p>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <table class="table table-striped table-hover">
                            ...
                            <tbody>
                                <c:forEach var="f" items="${feedbackList}">
                                    <tr>
                                        <td><c:out value="${f.studentName}" /></td>
                                        <td><c:out value="${fn:substring(f.feedbackText, 0, 100)}${f.feedbackText.length() > 100 ? '...' : ''}" /></td>
                                        <td>
                                            <c:choose>
                                                <c:when test="${not empty f.feedbackDate}">
                                                    <fmt:parseDate value="${f.feedbackDate}" pattern="yyyy-MM-dd" var="parseDate" />
                                                    <fmt:formatDate value="${parseDate}" pattern="dd/MM/yyyy" />
                                                </c:when>
                                                <c:otherwise>
                                                    <c:out value="${f.feedbackDate}" default="N/A" />
                                                </c:otherwise>
                                            </c:choose>
                                        </td>
                                    </tr>
